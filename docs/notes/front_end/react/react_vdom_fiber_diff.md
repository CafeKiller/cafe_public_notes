---
title: "React 实现 vdom、fiber、diff"
category: WEB前端
subcategory: React
level: 6
tags:
...
# React 实现 vdom、fiber、diff

## 虚拟DOM

Virtual DOM 就是用 JavaScript 对象去描述一个 DOM 结构，虚拟DOM不是直接操作浏览器的真实 DOM，而是首先对 UI 的更新在虚拟 DOM 中进行，再将变更高效地同步到真实 DOM 中。
...
用于生成虚拟 DOM 树，返回一个包含 type（元素类型）和 props（属性和子元素）的对象。 children 可以是文本或其他虚拟 DOM 对象。

React.createTextElement: 用于处理文本节点，将字符串封装成虚拟 DOM 对象。

React.render: 将虚拟 DOM 转化为实际 DOM 元素。 使用递归的方式渲染所有子元素。 最后将生成的 DOM 节点插入到指定的容器中。
...
render(React.createElement('div', { id: 'root' }, React.createElement('span', null, '小满zs')), document.getElementById('root'));
