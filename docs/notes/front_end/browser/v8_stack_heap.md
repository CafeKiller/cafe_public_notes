...
> [!tip] 参考资料
>
> [浏览器工作原理与实践 - 极客空间](https://blog.poetries.top/browser-working-principle/)
...
第二个问题：innerBar返回后，含有 setName 和 getName 对象，这两个对象里面包含了堆中的closure(foo)的引用。虽然foo执行上下文销毁了，foo函数中的对closure(foo)的引用也断开了，但是setName和getName里面又重新建立起来了对closure(foo)引用。
...
- 搜索“setName”，然后你就会发现setName对象下面包含了 r`aw_outer_scope_info_or_feedback_metadata`，对闭包的引用数据就在这里面。`